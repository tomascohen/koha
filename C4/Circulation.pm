...
sub _FixAccountForLostAndReturned {
    ...
    if ( $accountline->amount > $accountline->amountoutstanding ) {
        ...
        # this works because the only way to substract from a debt is
        ...
        if ( $total_to_refund > 0 ) {
            ...
            $credit->apply({ debits => Koha::Account::Lines->search({ accountlines_id => $accountline->id }), offset_type => 'Lost Item' });
        }
